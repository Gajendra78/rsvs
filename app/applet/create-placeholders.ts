...
const layouts = [
  '/app/applet/app/(protected)/student/layout.tsx',
  '/app/applet/app/(protected)/admin/layout.tsx',
  '/app/applet/app/(protected)/manager/layout.tsx',
  '/app/applet/app/(protected)/faculty/layout.tsx'
];

const missingPages: { href: string, pagePath: string }[] = [];
...
layouts.forEach(layoutPath => {
  ...
  while ((match = regex.exec(content)) !== null) {
    const href = match[1];
    if (href.startsWith('/')) {
      const pagePath = path.join('/app/applet/app', href, 'page.tsx');
...
missingPages.forEach(({ href, pagePath }) => {
  ...
  const title = href.split('/').pop()?.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) || 'Page';
